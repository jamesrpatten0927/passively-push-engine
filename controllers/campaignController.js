...
exports.getPublicCampaigns = async (req, res) => {
  try {
    ...
    const campaignsResult = await pool.query(
      'SELECT * FROM spotlight_campaigns WHERE advisor_id = $1',
      [advisorId]
    );

    // Fetch only active schedules for this advisor
    const schedulesResult = await pool.query(
      'SELECT * FROM universal_schedules WHERE advisor_id = $1 AND status = $2',
      [advisorId, 'active']
    );
...
exports.getCampaigns = async (req, res) => {
  try {
    const { advisorId } = req.params;
    const result = await pool.query('SELECT * FROM spotlight_campaigns WHERE advisor_id = $1', [advisorId]);
...
exports.saveCampaign = async (req, res) => {
  try {
    const {
      id, advisorId, name, title, description, badgeText, buttonText,
      imageUrl, targetAction, targetId, inheritColor, customColor, scheduleId
    } = req.body;

    await pool.query(
      `INSERT INTO spotlight_campaigns 
       (id, advisor_id, name, title, description, badge_text, button_text, image_url, target_action, target_id, inherit_color, custom_color, schedule_id, updated_at) 
      ...
      [id, advisorId, name, title, description, badgeText, buttonText, imageUrl, targetAction, targetId, inheritColor, customColor, scheduleId]
    );
...
exports.getSchedules = async (req, res) => {
  try {
    const { advisorId } = req.params;
    const result = await pool.query('SELECT * FROM universal_schedules WHERE advisor_id = $1', [advisorId]);
...
exports.saveSchedule = async (req, res) => {
  try {
    const {
      id, advisorId, name, targetType, targetId, startDate, endDate, timezone, recurrence, status
    } = req.body;

    await pool.query(
      `INSERT INTO universal_schedules 
       (id, advisor_id, name, target_type, target_id, start_date, end_date, timezone, recurrence, status, updated_at) 
